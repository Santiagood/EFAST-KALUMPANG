<div class="w-full max-w-full p-3 text-left bg-white border border-gray-200 rounded-lg shadow-md sm:p-2 md:p-4 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-2 py-2 text-center">
                            @if ($Account -> email == null)
                                <span class="inline-block px-2 py-2 mr-1 text-xs font-semibold text-gray-600 uppercase bg-gray-200 rounded-full last:mr-0">
                                    created manually
                                </span>
                            @else
                                {{ $Account -> email }}
                        ...
                        <td class="px-2 py-2 text-center md:whitespace-nowrap">
                            <button wire:click='updateShowModal( {{ $Account -> id }})' type="submit" class="text-gray-900 bg-gradient-to-r from-teal-200 to-lime-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
                                {{ __('Update') }}
                            </button>

                            <button wire:click='deleteShowModal({{ $Account -> id }})' type="submit" class="text-white bg-gradient-to-br from-pink-500 to-orange-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-pink-200 dark:focus:ring-pink-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
                                {{ __('Delete') }}
                            </button>
                        </td>
                    </tr>
    ...
    <x-jet-dialog-modal wire:model="modalCreateVisible" >
        ...
        <x-slot name="footer">
            <button wire:click="$toggle('modalCreateVisible')" wire:loading.attr="disabled" type="submit" class="text-gray-900 bg-gradient-to-r from-red-200 via-red-300 to-yellow-200 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-red-100 dark:focus:ring-red-400 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
                {{ __('Cancel') }}
            </button>

            <button wire:click="create" wire:loading.attr="disabled" type="submit" class="text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
                {{ __('Create Account') }}
            </button>
        </x-slot>
    </x-jet-dialog-modal>
    ...
    <x-jet-dialog-modal wire:model="modalUpdateVisible">
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                <x-jet-label for="Residents_Gender" value="{{ __('Resident Gender') }}" />
                <select id="Residents_Gender" name="Residents_Gender" wire:model.debounce.800ms="Residents_Gender" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    <option value="">Select Gender</option>
                    <option value="Male">Male</option>
                    <option value="Female">Female</option>
                    <option value="I Prefer Not To Say">I prefer not to say</option>
            ...
            <div class="mt-4">
                <x-jet-label for="Residents_Birthday" value="{{ __('Resident Birthday') }}" />
                <x-jet-input id="Residents_Birthday" name="Residents_Birthday" wire:model.debounce.800ms="Residents_Birthday" class="block w-full mt-1 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="date" name="birthday" :value="old('birthday')" placeholder="" required />
                @error('Residents_Birthday')<span class="text-sm font-semibold text-red-500">{{ $message }}</span>@enderror
            </div>
        ...
        <x-slot name="footer">
            <button wire:click="$toggle('modalUpdateVisible')" wire:loading.attr="disabled" type="submit" class="text-gray-900 bg-gradient-to-r from-red-200 via-red-300 to-yellow-200 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-red-100 dark:focus:ring-red-400 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
                {{ __('Cancel') }}
            </button>
    ...
    <x-jet-dialog-modal wire:model="modalDeleteVisible">
        ...
        <x-slot name="footer">
            ...
            <button wire:click="$toggle('modalDeleteVisible')" wire:loading.attr="disabled" type="submit" class="text-gray-900 bg-gradient-to-r from-red-200 via-red-300 to-yellow-200 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-red-100 dark:focus:ring-red-400 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">
                {{ __('Cancel') }}
            </button>
