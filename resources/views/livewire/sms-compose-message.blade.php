<div class="w-full max-w-full p-3 text-left bg-white border border-gray-200 rounded-lg shadow-md sm:p-2 md:p-4 dark:bg-gray-800 dark:border-gray-700">
    <div class="space-y-6">
        <p class="font-bold text-center">Compose SMS Alert</p>
        ...
        <div>
            <form action="{{ route('BarangayOfficials.SendItexmo.sendSMS') }}" method="post">
                @csrf
                <label for="message" class="block mb-2 text-sm font-mono text-blue-600 dark:text-gray-400">Compose an SMS message, or choose a template above then edit</label>
                <div class="flex items-center py-2 px-3 bg-gray-50 rounded-lg dark:bg-gray-700">
                <textarea id="message" name="message" type="text" rows="9.5" class="block h-52 p-2.5 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">{{ $selectedTemplate }}</textarea>
